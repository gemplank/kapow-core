...
trait Trait_Render_View {
	...
	public static function render_view( $file_name ) {
		...
		$view_template_folder              = apply_filters( KAPOW_CORE_PREFIX . '_view_template_folder', '' );
		$view_template_folder_check_exists = apply_filters( KAPOW_CORE_PREFIX . '_view_template_folder_check_exists', false );

		// Use the `_view_template_folder` filter to check for a custom location,
		// eg: `get_stylesheet_directory() . '/template-parts/kapow-core/'`
		//
