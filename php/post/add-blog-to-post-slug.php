 ...
 */
function kapow_core_remove_default_post_type() {
	remove_menu_page( 'edit.php' );
}
add_action( 'admin_menu', 'kapow_core_remove_default_post_type' );

/**
 * Re-add the 'post' post type
 */
function kapow_core_add_blog_to_post_slug() {
	$labels = array(
...
add_action( 'init', 'kapow_core_add_blog_to_post_slug', 1 );

/**
 * Handle conflicts with categories now the posts use the slug /blog.
 */
function kapow_core_add_blog_to_post_slug_routing() {
	...
	if ( ! is_admin() && is_404() ) {
		...
		if ( false !== stripos( $url, 'blog' ) ) {
			...
			if ( ! empty( $name ) ) {
				$post = get_page_by_path( $name, OBJECT, $this->post_type );
				...
				if ( is_object( $post ) ) {
					...
					if ( ! empty( $query->posts ) ) {
						...
						$GLOBALS['wp_query']         = $wp_query = $query;
						$GLOBALS['wp_the_query']     = $GLOBALS['wp_query'];
					}
				}
			}
...
add_action( 'wp', 'kapow_core_add_blog_to_post_slug_routing', 99 );
