 ...
 */
function kapow_core_sanitize_post_content( $data, $postarr ) {

	// Clean up the post content.
	$data['post_content'] = kapow_core_sanitize_html( $data['post_content'] );

	// Clean up the post excerpt.
	$data['post_excerpt'] = kapow_core_sanitize_html( $data['post_excerpt'] );

	return $data;
}
add_filter( 'wp_insert_post_data' , 'kapow_core_sanitize_post_content', 99, 2 );
 ...
 */
function kapow_core_sanitize_post_meta( $post_id ) {
	 ...
	 * // Get the post meta.
 	 ...
 	 * $meta_example = kapow_core_sanitize_html( $meta_example );
 	 *
 	 * // Save the meta.
	 ...
	 * // Get the post meta (this should be an array).
 	 ...
 	 *    // Sanitize the items HTML key.
 	 *    $item['content'] = kapow_core_sanitize_html( $item['content'] );
 	 * }
 	 *
 ...
 */
function kapow_core_sanitize_html( $content ) {
 ...
 * @return string
 */
function kapow_core_remove_empty_p( $content ) {
...
add_filter( 'the_content', 'kapow_core_remove_empty_p', 20, 1 );
 ...
 * @return string
 */
function kapow_core_remove_image_ptags( $content ) {
	return preg_replace( '/<p>\s*(<a .*>)?\s*(<img .* \/>|<iframe .*>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $content );
}
add_filter( 'the_content' , 'kapow_core_remove_image_ptags' );
